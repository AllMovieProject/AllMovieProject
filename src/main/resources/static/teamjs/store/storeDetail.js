...
const useStoreDetailStore = defineStore('storeDetail', {
  ...
  getters: {
      ...
      if (state.product.pvo.is_combo === 'Y') {
        state.comboItems.forEach(comboItem => {
          ...
          for (let i = 0; i < comboItem.item_quantity; i++) {
            ...
            if (selectedItemId) {
              const selectedItem = comboItem.upgradeOptions?.find(opt => opt.item_id == selectedItemId)
              const baseItem = comboItem.upgradeOptions?.find(opt => opt.item_id == comboItem.item_id)
              
              if (selectedItem && baseItem) {
                // 선택된 상품 가격 - 기본 상품 가격
                const priceDiff = (selectedItem.item_price + (selectedItem.add_price || 0)) - 
    ...
    discountedPrice: (state) => {
      ...
      return state.totalQuantity * (state.totalPrice / state.totalQuantity - discount)
    }
  },
  ...
  actions: {
    async loadProductDetail(storeId, productId) {
      this.loading = true
      try {
        const { data } = await api.get('/store/' + storeId + '/product/' + productId)
        this.product = data
    ...
    async loadCategoryOptions(baseItemId) {
      try {
        const { data } = await api.get('/product/upgrade-options/' + baseItemId)
        return data
      } catch (error) {
    ...
    getSelectedOptions() {
      ...
      this.comboItems.forEach(comboItem => {
        for (let i = 0; i < comboItem.item_quantity; i++) {
          ...
          const selectedOption = comboItem.upgradeOptions?.find(opt => opt.item_id == selectedItemId)
